...
namespace NeoNetsphere.Game.GameRules
{
  ...
  internal sealed class TouchdownTrainingGameRule : GameRuleBase
  {
    ...
    private bool IsAssistValid => _lastOwner != 0 && DateTime.Now - LastTime < TouchdownAssistTimer &&
                                  !_lastOwner.EqualSlot(_ballOwner);

    private TimeSpan _touchdownTime;

    private bool IsInTouchdown { get; set; }

    public override bool BlockPlaying => IsInTouchdown;
    ...
    public void OnScoreRebound(Player newPlr, Player oldPlr, LongPeerId newid, LongPeerId oldId)
    {
      ...
      /* Disable assist due to unable to check teams
        if (oldPlr != null)
        {
            _lastPlayer = oldPlr;
        }
