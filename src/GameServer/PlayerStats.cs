...
namespace NeoNetsphere
{
  ...
  internal abstract class BaseStats
  {
    ...
    public float WinRate => _won + _loss > 0 ? _won / (float)(_won + _loss) : 0.5f;
  ...
  internal class DMStats : BaseStats
  {
    ...
    public override void Save(IDbConnection db)
    {
      ...
      {
        DbUtil.Update(db, update);
      }
      else
      {
        _existsInDatabase = true;
        DbUtil.Insert(db, update);
      }
    }
  ...
  internal class TDStats : BaseStats
  {
    ...
    public ulong TotalScore => 5 * (TD * 2 + TDAssist) +
                               2 * (_kills + OffenseAssist + DefenseAssist + OffenseRebound +
                                    2 * (Offense + Defense)) + _killAssists + Heal;
